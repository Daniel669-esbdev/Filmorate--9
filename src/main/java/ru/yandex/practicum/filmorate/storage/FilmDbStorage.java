...
public class FilmDbStorage implements FilmStorage {
    ...
    private void updateDirectorsByFilmId(long filmId, List<Director> directors) {
        ...
        // Получаем всех режиссеров по ids
        String getDirectorsByIdsSql = """
                SELECT *
                FROM directors
                WHERE id IN (:ids)
                """;
        ...
        MapSqlParameterSource params = new MapSqlParameterSource().addValue(
                "ids",
                directors.stream().map(director -> director.getId()).toList());
        List<Director> updatedDirectors = namedParameterJdbcTemplate.query(
                getDirectorsByIdsSql,
                params,
                directorRowMapper);
    ...
    public List<Film> findAllBy(Integer directorId) {
        String selectFilmDirectorByDirectorIdSql = """
                SELECT film_id
                FROM film_director
                WHERE director_id = ?
                """;
        List<Integer> idsForFilms = jdbcTemplate.queryForList(
                selectFilmDirectorByDirectorIdSql,
                Integer.class,
                directorId);
